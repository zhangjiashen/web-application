...
public class TweetAnalysis {
  ...
  public static class RawJsonFilterMapper
...
//    private final static IntWritable one = new IntWritable(1);
    ...
    public static int getScore(String[] words){
		int score = 0;
		for (String word : words){
			if (word != null && !word.isEmpty()){
				if (map.containsKey(word.toLowerCase())){
					score += map.get(word.toLowerCase());
				}
			}
		}
		return score;
	...
	public static String censor(String text){
		...
		for (i=0; i < text.length(); i++){
			c = text.charAt(i);
			sb.append(c);
			if (Character.isAlphabetic(c) || Character.isDigit(c)){//( c < '0') || (c > '9' && c < 'A') || (c > 'Z' && c < 'a') || (c > 'z')
				len ++;
			} else{
				if (len > 0){// a word has end
					word = text.substring(i-len, i);
...
//      }
    	BufferedReader bufferedRdr = new BufferedReader(new StringReader(value.toString()));
    	String line = null;
    	init();
		Gson gson = new Gson();
		StringReader sr;
    	...
    	while ((line = bufferedRdr.readLine()) != null) {
			...
			//count++;
			try{
				tweet = gson.fromJson(sr, Tweet.class);
				timestamp = tweet.created_at.split(" ");
				tweet.created_at = timestamp[5] + "-" + month(timestamp[1]) + "-" + timestamp[2] + " " + timestamp[3];
				
				String[] words = tweet.text.split("[^a-zA-Z0-9]");
				score = getScore(words);//compute score
				
				tweet.text = censor(tweet.text);//censoring
				
				CensoredTweet censoredTweet = new CensoredTweet(tweet.created_at, tweet.text, tweet.user.id_str, score);
				mapkey = tweet.id_str;
				mapvalue = gson.toJson(censoredTweet);
				//fw.write(key + " ");
				//fw.write(value + "\n");				
			} catch (Exception e) {
    		...
    		context.write(tweetId, result);
    	}
    }
  }

  public static class TextAnalysisReducer
  ...
  public static void main(String[] args) throws Exception {
    ...
    Job job = Job.getInstance(conf, "tweet analysis");
    job.setJarByClass(TweetAnalysis.class);
    job.setMapperClass(RawJsonFilterMapper.class);
